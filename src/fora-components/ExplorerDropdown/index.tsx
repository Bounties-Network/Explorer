...
import Select, { Props as RSProps, StylesConfig, components } from 'react-select'
...
const YourCommunitiesOption = (props) =>
  <Flex {...props} {...props.innerProps} sx={{
    ...optionStyles, alignItems: 'center', '> svg': { mr: 2 },
...
const CommunityOption = (props) => (
  <Flex {...props} {...props.innerProps} sx={{
    ...optionStyles, '> a': {
      ...
      '&:hover': {
        color: theme.colors.brandPrimary[300], '*': { color: theme.colors.brandPrimary[300] },
      }
    }
  }}>
...
const GroupHeading = props => (
  <div sx={{ py: 2, px: 4}}>
    <Text color="brandGray.400" variant='label' sx={{ fontFamily: 'body' }}>{props.children}</Text>
  </div>
);
...
const ExplorerDropdown: React.FunctionComponent<RSProps> = props => {
  ...
  const optionsLength = Array.isArray(props.options) &&
    props.options.reduce((current, next) => current + next?.options.length || 0, 0)
