...
const AddProfileDetails = props => {
  const { visible, onClose, showForm, form } = props;

  if (form) {
    return (
      <Modal visible={visible} size="large" dismissable onClose={onClose}>
        <Modal.Header closable icon={['fal', 'faAddressCard']}>
  ...
  return (
    <Modal visible={visible} size="small" dismissable onClose={onClose}>
      <Modal.Header closable icon={['fal', 'idCardAlt']}>
        <Modal.Heading>Profile Details</Modal.Heading>
      </Modal.Header>
      <Modal.Body>
        <Text>
